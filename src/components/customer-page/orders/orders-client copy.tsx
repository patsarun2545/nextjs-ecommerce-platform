...
import {
  ...
  XCircle, Clock, CreditCard, Search, X, Hash, SlidersHorizontal,
} from "lucide-react";
...
const STATUS_CONFIG: Record<OrderStatus, {
  label: string; icon: React.ElementType; cls: string; step: number;
}> = {
...
export default function MyOrdersClient({ orders }: { orders: Order[] }) {
  ...
  // Sidebar = status filter list (mirrors products sidebar style)
  const SidebarContent = () => (
    <div className="space-y-3">
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">สถานะออเดอร์</p>
        </div>
        <ul className="p-2 space-y-0.5">
          {STATUS_FILTERS.map((f) => {
            ...
            return (
              <li key={f.value}>
                <button
                  onClick={() => { setActiveStatus(f.value); setMobileSidebarOpen(false); }}
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all duration-100 ${
                    isActive
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 xl:px-0 py-5">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            {/* Search — identical markup to products */}
      ...
      <div className="max-w-7xl mx-auto px-4 xl:px-0 py-5">
        ...
        {/* Active filter chip — mirrors products chip bar */}
        {hasFilters && (
        ...
        <div className="flex gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Toolbar — mirrors products toolbar */}
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                {/* Mobile filter button */}
                <button
                  onClick={() => setMobileSidebarOpen(true)}
                  className="lg:hidden inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm text-slate-600 hover:border-blue-300 hover:text-blue-600 transition-colors"
                >
                  <SlidersHorizontal size={13} />
                  ตัวกรอง
                  {hasFilters && (
                    <span className="size-4 rounded-full bg-blue-600 text-white text-[9px] font-bold flex items-center justify-center">!</span>
                  )}
                </button>
                {/* Active status label */}
                {activeStatus !== "all" && (
                  <span className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border text-xs font-semibold ${STATUS_CONFIG[activeStatus as OrderStatus]?.cls}`}>
                    {(() => { const C = STATUS_CONFIG[activeStatus as OrderStatus]?.icon; return C ? <C size={11} /> : null; })()}
                    {STATUS_FILTERS.find(f => f.value === activeStatus)?.label}
                  </span>
                )}
              </div>
              <span className="text-xs text-slate-400 hidden sm:block">
                แสดง <span className="font-semibold text-slate-600">{filtered.length}</span> / {orders.length} รายการ
              </span>
            </div>
      ...
      {/* Mobile Drawer — identical to products */}
      {mobileSidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-64 h-full bg-slate-50 shadow-2xl overflow-y-auto flex flex-col">
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-slate-200 bg-white sticky top-0">
              <p className="font-bold text-slate-800 text-sm">กรองสถานะ</p>
              <button
                onClick={() => setMobileSidebarOpen(false)}
                className="size-7 flex items-center justify-center rounded-lg bg-slate-100 hover:bg-slate-200 transition-colors"
              >
...
// ── Order Card — mirrors ListCard style from products ──────────────────
function OrderCard({ order }: { order: Order }) {
  ...
  return (
    ...
    >
      ...
      {/* Progress steps — only when not cancelled */}
...
// ── Chip — identical to products Chip ─────────────────────────────────
